...
- [readme] fix travis badges, https all URLs [`e516a73`](https://githuReadSubtree() can be called only if the reader is on an element node.   The '{0}' property is read only and cannot be set.  Set XmlReaderSettings.Async to true if you want to use Async Methods.   Content cannot be converted to the type {0}.    {0} method is not supported on this XmlReader. Use CanReadBinaryContent property to find out if a reader implements it. Cannot use a prefix with an empty namespace.    XmlNodeType {0} is not supported for partial content parsing.   This NodeType is not supported for partial content parsing. The numeric entity value is too large.  Resolving of external URIs was prohibited.  No validation occurred. WriteStartDocument needs to be the first call.  Not the same name table.    NotInWriteState.    The buffer is not large enough to fit a surrogate pair. Please provide a buffer of size at least 2 characters.  There was no XML start tag open.    Document does not have a root element.  Only white space characters should be used. Cannot set the namespace if Namespaces is 'false'.  Document cannot have multiple document elements.    No DTD found.   XmlReaderSettings.XmlNameTable must be the same name table as in XmlParserContext.NameTable or XmlParserContext.NamespaceManager.NameTable, or it must be null. Prefix '{0}' cannot be ma